...
class Memcache
{
     ...
     * @return Memcache
     */
    public static function getInstance($params = []): Memcache
    {
        if (empty($params)) {
            $params = [
                'host' => 'localhost',
        ...
        if (!self::$connectedServers[$serverId]) {
            ...
            if (!$server->connect($params['host'], $params['port'])) {
                Error::add("Can't connect to memcache");
            }
     ...
     * @return bool Возвращает true при успешном выполнении и false в случае ошибки
     */
    public function safeIncrement($key, $value = 1, $ttl = false): ?bool
    {
        if ($this->memcacheWrapper !== null) {
     ...
     * @return bool Возвращает true при успешном выполнении и false в случае ошибки
     */
    public function safeDecrement($key, $value = 1, $ttl = false): bool
    {
        if ($this->memcacheWrapper !== null) {
